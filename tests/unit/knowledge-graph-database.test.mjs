...
test("database opens lazily, applies metadata migration, configures pragmas, and closes idempotently", () => {
  ...
  try {
    ...
    assert.equal(connection.prepare("SELECT COUNT(*) AS count FROM schema_migrations").get().count, 7);
...
test("migrations are ordered and create a verified pre-migration backup", () => {
  ...
  try {
    ...
    const migrationEight = migration(8, "add_migration_probe", (database) => {
    ...
    const backupPath = join(config.backupDirectory, "migration-v7-v8.sqlite");
    const upgraded = new KnowledgeGraphDatabase({
      paths: config,
      now: () => 1_700_000_000_100,
      migrations: [...MVP_MIGRATIONS, migrationEight],
    ...
    assert.equal(backup.prepare("SELECT COUNT(*) AS count FROM schema_migrations").get().count, 7);
...
test("failed migrations roll back and leave the original schema recoverable", () => {
  ...
  try {
    ...
    const failedMigration = migration(8, "failed_probe", (database) => {
    ...
    assert.equal(recovered.getSchemaVersion(), 7);
